...
description: "Use when checking Java code quality against Alibaba P3C rules including naming, exceptions, concurrency, database, OOP, security, and unit tests."
---
# P3C代码质量检查测试
...
### 基本测试流程
1. **查找 Java 文件**：使用 `rg --files` 工具查找所有 `.java` 文件
2. **读取代码内容**：使用 Codex 文件读取工具 工具读取文件内容
3. **代码结构分析**：使用 `SearchCodebase` 进行代码结构分析
4. **获取编译诊断**：使用 `GetDiagnostics` 获取编译错误和警告
5. **正则匹配检查**：使用 `rg` 工具配合正则表达式匹配 P3C 规范违规
6. **调用子技能**：调用以下子技能进行详细检查
   ...
   - p3c-security-rules
   - p3c-unit-testing
7. **生成测试报告**：使用 Codex 文件编辑工具 工具生成 Markdown 格式报告

### 常用 Trae IDE 内置工具

**文件查找工具**
- `rg --files` - 查找所有 `.java` 文件
- `LS` - 列出目录结构
...
**代码分析工具**
- `rg` - 使用正则表达式搜索代码，匹配 P3C 规则
- `GetDiagnostics` - 获取 VS Code 的语言诊断信息（编译错误、警告等）

**报告生成工具**
- Codex 文件编辑工具 - 生成 Markdown 格式的测试报告
...
- 使用 `rg --files` 查找所有 `.java` 文件
- 使用 Codex 文件读取工具 读取文件内容
...
### 步骤4：生成测试报告
使用 Codex 文件编辑工具 工具生成详细的测试报告，包含：
- 测试范围和测试时间
- 问题统计（按严重程度和规范类别）
- 问题详情列表
- 改进建议

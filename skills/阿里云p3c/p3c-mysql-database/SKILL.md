...
description: "Use when designing MySQL tables, writing SQL, creating indexes, optimizing queries, or mapping ORM code under Alibaba P3C database rules."
---
# P3C MySQL数据库规范
...
1. 【强制】MySQL 8.x 版本必须保持 `sql_mode` 为默认值，禁止修改。默认值为：`ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION`
2. 【强制】禁止通过 `SET GLOBAL sql_mode` 或 `SET SESSION sql_mode` 修改 SQL 模式，避免引发隐式类型转换、零日期兼容等问题

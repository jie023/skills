...
5. **强制调用子 skill**：
   ...
   - **必须**优先使用当前平台可用的子代理执行各阶段专项检查；子代理不可用时，按“子代理优先调度”规则记录原因并回退到本地检查。
...
- 每个检查阶段开始前，必须先判断当前平台是否具备子代理、多代理、Task、Agent、SubAgent 或等价调度能力；具备时必须优先把本阶段适用的专项检查交给子代理执行。
- 子代理任务必须按专项拆分，任务说明必须包含检查范围、对应子 skill 名称、只做代码静态检查、不跑接口、不自动修复、输出文件路径/行号/证据/等级/建议。
- 子代理只允许在同一阶段内并行处理相互独立的专项；不同阶段仍必须遵守阶段串行门禁，不得跨阶段并行推进。
- 子代理返回后，主流程必须复核并汇总其结论，再写入 Markdown 报告；不得直接照搬无法落到文件路径和行号的结论。
- 如果当前平台没有可用子代理，或子代理调用失败，必须在 Markdown 报告中记录“子代理不可用/调用失败”、失败原因和替代检查方式，然后使用当前平台可用工具继续本地检查。

### 阶段串行门禁（强制）

- **必须等待上一阶段全部完成后，才能开始下一阶段**；不得并行推进多个阶段，不得在上一阶段仍有待调用、待汇总或待更新报告事项时提前进入下一阶段。
- 阶段完成判定：本阶段适用子 skill 已全部调用；不可用子 skill 已记录原因并完成替代检查；该阶段问题、证据、统计和无法确认项已同步更新到 Markdown 报告。
- 如果上一阶段发现阻断或高风险问题，仍需先完成该阶段报告汇总，再进入下一阶段；不得因为发现严重问题而跳过剩余适用检查。
- 进入下一阶段前，必须在对话检查清单和 Markdown 报告中明确标记上一阶段为“已完成”。
...
**阶段门禁要求：**每个阶段开始前必须确认上一阶段清单已全部完成；若存在不适用项，必须在报告中说明“不适用原因”；若存在不可用项，必须完成替代检查并记录后，才允许进入下一阶段。
